...
If the P/E is negative, E must be negative because the numerator can be assumed to be positive. As discussed, if E is negative then inverted min-max normalisation cannot be used for scoring purposes and, instead, each negative E is treated as a very low positive E because it's the nearest scorable value. Since P is assumed positive and non-extremised, treating E as a very low positive value makes the P/E very high. Therefore, when the P/E is negative, it's treated as a very high value and receives the minimum score.

In score_for_peg(), the following logic is applied when handling P/EG values with a negative E and/or negative G:

Since the numerator can be assumed to be positive, the following can be deduced: if P/EG is negative and P/E is positive then expected earnings growth is negative and earnings are positive. If P/EG is negative and P/E is negative then expected earnings growth is positive and earnings are negative. If P/EG is positive and P/E is negative then expected earnings growth and earnings are both negative. For the same reason as when scoring the P/E, any negative denominator variable is treated as a very small positive value. In each case, this results in a very small positive denominator product and a very high P/EG value. Therefore, when the P/EG and/or the P/E is negative, the P/EG is treated as a very high value and receives the minimum score.

In score_for_ev_to_revenue(), the following logic is applied when handling EV/R values with a negative EV:

The reason that inverted min-max normalisation cannot be used to score an EV/R value with a negative EV is not immediately obvious. Unlike in the P/E case, where E transitioning from a low positive value to a negative value causes the P/E to flip from a very large positive value to a negative value, the EV/R remains low when EV is either a low positive value or a negative value. This is because the denominator is changing sign in the P/E case, causing the ratio to become discontinuous when it passes through zero, whereas the numerator is changing sign in the EV/R case, causing the ratio to pass continuously through zero.

However, despite this difference, inverted min-max normalisation cannot be used when EV is negative because a negative EV reverses the relationship between R and EV/R. When EV is negative, the EV/R value increases as R increases. This is the opposite relationship to positive EV, where EV/R decreases as R increases.

Therefore, inverted min-max normalisation cannot be used when EV is negative. Instead, treatment analogous to the P/E and P/EG cases is used. When a denominator variable has a negative value in the P/E and P/EG cases, it is treated as the nearest value that creates a scorable ratio: a very small positive value. Likewise, a negative EV is treated as the nearest value that creates a scorable ratio: zero EV. Since R can be assumed positive, when EV/R is negative then EV is negative. Thus, when EV/R is negative, it's treated as zero and assigned the maximum score.
...
There are three cases for EV/EBITDA that require separate handling: negative EV and positive EBITDA, positive EV and negative EBITDA, negative EV and negative EBITDA. For the same reason as when scoring the EV/R, if EV is negative and EBITDA is positive then the EV/EBITDA is treated as zero and receives the maximum score. For the same reason as when scoring the P/E, if EV is positive and EBITDA is negative then the EV/EBITDA is treated as a very high value and receives the minimum score. The framework used so far to handle cases that cannot be scored using inverted min-max normalisation is that a negative numerator is treated as zero and a negative denominator is treated as a very low positive denominator. Extending this logic to the case where EV is negative and EBITDA is negative, the EV is treated as zero and the EBITDA is treated as a very low positive value. However, this differs from the cases handled so far, where exactly one of the numerator and denominator is mapped to its nearest scorable value. In these cases, the other term in the ratio has always been non-extremised, making the ratio's behaviour forced. When both the numerator and denominator are mapped to their nearest scorable values, there is no anchor term from which the behaviour of the ratio can be deduced. Instead, all that can be established is that both the numerator and denominator values are invalid for scoring and treating them as their closest scorable values would require making the numerator zero and the denominator a very small positive value. This matters because a ratio is defined by the relative magnitude and sign of its numerator and denominator. When they are both treated as their nearest scorable value, information about their relative magnitude is lost because nothing is known about their respective proximities to the closest scorable cases: one term's true negative value may lie much nearer to the closest scorable value than the other's. On this basis, the true relative magnitude of EV and EBITDA becomes unclear once both are mapped to their nearest scorable values, so the resulting EV/EBITDA value of 0 cannot be considered a meaningful estimate of the company's actual EV/EBITDA. Therefore, the EV/EBITDA is ignored and, instead, this case is scored by manually incrementing the total by 0.5 points. This is considered a neutral score to award in this scenario, reflecting the loss of information rather than any specific claim about the company's valuation.
